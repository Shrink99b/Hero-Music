...
from Hero import YouTube, app
from Hero.core.call import Veer
from Hero.misc import SUDOERS, db
from Hero.utils.database import (is_active_chat,
                                       is_music_playing, set_loop,
                                       is_nonadmin_chat, music_off,
                                       music_on)
from Hero.utils.decorators.language import languageCB
from Hero.utils.formatters import seconds_to_min
from Hero.utils.inline.play import (panel_markup_1,
                                          stream_markup,
                                          telegram_markup)
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await Veer.pause_stream(chat_id)
        ...
        await CallbackQuery.answer()
        await Veer.stop_stream(chat_id)
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await Veer.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                await Veer.skip_stream(chat_id, link, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            try:
                await Veer.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
