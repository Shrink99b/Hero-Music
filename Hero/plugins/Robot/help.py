...
from Hero.utils import help_pannel
from Hero.utils.database import get_lang, is_commanddelete_on
from Hero.utils.decorators.language import (LanguageStart,
                                                  languageCB)
from Hero.utils.inline.help import (help_back_markup,
                                          private_help_panel)
    ...
    if cb == "hb9":
        if CallbackQuery.from_user.id not in SUDOERS:
            return await CallbackQuery.answer(
                "sᴜᴅᴏ ᴜsᴇʀs ᴏɴʟʏ.", show_alert=True
            )
        else:
