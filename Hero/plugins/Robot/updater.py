...
@Client.on_message(filters.command(["update"]) & filters.user(OWNER_ID) & ~filters.edited)
async def update_bot(_, message: Message):
    chat_id = message.chat.id
    msg = await message.edit("**🥀 ᴄʜᴇᴄᴋɪɴɢ ᴜᴘᴅᴀᴛᴇs ✨ ...**")
    update_avail = updater()
    if update_avail:
        await msg.edit("**🥀 Uᴘᴅᴀᴛᴇᴅ Tᴏ Lᴀᴛᴇsᴛ Vᴇʀsɪᴏɴ 🔥 ...\n\n💕 Rᴇsᴛᴀʀᴛɪɴɢ Bᴏᴛ, Pʟᴇᴀsᴇ Wᴀɪᴛ ✨ ...**")
        system("git pull -f && pip3 install -U -r requirements.txt")
        system("python3 -m Hero")
        return
    await msg.edit(f"**🥀 Bᴏᴛ Aʟʀᴇᴀᴅʏ Uᴘᴅᴀᴛᴇᴅ Tᴏ Lᴀᴛᴇsᴛ 🔥 ...**")
